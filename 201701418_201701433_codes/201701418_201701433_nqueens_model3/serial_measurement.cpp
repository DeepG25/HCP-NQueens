...
long long int threadCount[100];				//used to stores the solution count by each thread 
											//in case to serial execution only thread 0 contains value
const int LEVEL = 1;
...
void solveLinear(int board[],int boardSize,int row,int col,int col1)
{
	...
	else
	{
		...
		{
			solveLinear(board,boardSize,row+1,i,col1);		//recursive call to the next row
		}
	}
}
...
void solveLevel(int board[],int boardSize,int row,int col,int col1)
{
	...
	else
	{
		if(row <= LEVEL)
		{
			for(int i=0;i<boardSize;i++)
			{
				solveLevel(board,boardSize,row+1,i,col1);		//recursive call to the next row
			}
		}
		else
		{
...
void solveNQueens(int n)
{
	if(n%2 == 0)
	{
		for(int i=0;i<n/2 - 1;i++)				//n/2 board is visited using method of linear distribution
		{
			int *board = new int[n];
			solveLinear(board,n,0,i,i);
		}
		for(int i=n/2 - 1;i<n/2;i++)			//remaining board is visited using method of level distribution
		{
			int *board = new int[n];
			solveLevel(board,n,0,i,i);
		}
	}
	else
	{
		for(int i=0;i<n/2;i++)					//n/2 board is visited using method of linear distribution
		{
			int *board = new int[n];
			solveLinear(board,n,0,i,i);
		}
		for(int i=n/2;i<=n/2;i++)				//remaining board is visited using method of level distribution
		{
			int *board = new int[n];
			solveLevel(board,n,0,i,i);
		}
...
//	FILE* inputFile;
    ...
    for(int i=0;i<100;i++)
    {
    	...
    	totalSolutionCount += threadCount[i];		//adds solutions obtained by all thread in case of serial only thread 0
    }
