...
int solveBoard(int level,int row,int left_diagonal,int right_diagonal,int mask)
{
	...
	while(bitmap)
	{
		p = bitmap & (-bitmap);
		bitmap -= p;
		sum += solveBoard(level+1,row|p,(left_diagonal|p)<<1,(right_diagonal|p)>>1,mask);
	}
	return sum;
}

void solveNQueens(int n,int processors)
{
  int chunk = 1;
  int row = 0;
  int left_diagonal = 0;
  ...
  #pragma omp parallel for schedule(dynamic,chunk) reduction(+:totalSolutionCount)		//using reduction of calculate total sum
  for(int i=0;i<n;i++)
  {
  	if((bitmap>>i) & 1)
  	{
