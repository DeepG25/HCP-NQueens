...
int totalSolutionCount = 0;         //used to store total number of solutions
...
void solveBoard(int board[],int boardSize,int row,int col)
{
  ...
  board[row] = col;
  if(row == boardSize - 1)      //once all N queens are placed this condition holds true
  {
    #pragma omp critical
    {
      totalSolutionCount++;
  ...
  for(int i=0;i<boardSize;i++)
  {
    #pragma omp task
    {
      solveBoard(board,boardSize,row+1,i);        //recursive call for the next row
    }
  }
}
...
{
  omp_set_num_threads(p);         //using p processors to execute the algorithm
  #pragma omp parallel
  #pragma omp single
  {
    for(int i=0;i<n;i++)
